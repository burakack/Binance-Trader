...
def printinfo():
    macdValue = MACDvalue.getMacdValue(
        os.getenv('parite'), os.getenv('zamandilimi'))
    ...
    data = [[os.getenv('parite'), rsi, macd, macdValue, BBANDS[0]
             [-1], BBANDS[1][-1], BBANDS[2][-1], ma50, ma100, ma200, price]]
    clear()
    printdate()
    print(tabulate(data, headers=["Parite", "RSI", "MACD", "MACDVALUE",
          "BUP", "BMİD", "BDOWN", "MA50", "MA100", "MA200", "PRİCE"]))
...
def printinfobollinger():
    ...
    clear()
    printdate()
    print(tabulate(data, headers=["Parite", "RSI",
          "MACDVALUE", "BUP", "BMİD", "BDOWN", "PRİCE"]))
